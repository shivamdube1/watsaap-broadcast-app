...
- **Global scope**: `E:/watsaap broadcast/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/watsaap broadcast/.codex/notes/`)
...
2. Glob `E:/watsaap broadcast/.codex/notes/*.md` (if directory exists) — global notes
...
Global (E:/watsaap broadcast/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/watsaap broadcast/.codex/notes/` — works in any directory
