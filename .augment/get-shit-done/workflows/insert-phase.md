...
```bash
INIT=$(node "E:/watsaap broadcast/.augment/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "E:/watsaap broadcast/.augment/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
